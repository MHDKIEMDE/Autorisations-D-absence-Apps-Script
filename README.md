...
PERSONNEL: {
  presidents: {
    PRES_GENERAL: { email: 'president@…',     nom: 'Président Massaka SAS' },
    PRES_SAF:     { email: 'president.saf@…', nom: 'Président SAF'         }
  },
  presidentCroise: {        // qui valide un président qui demande lui-même
    PRES_GENERAL: 'PRES_SAF',
    PRES_SAF:     'PRES_GENERAL'
  },
  superieurs: {
    SUP_CPD:       { email: 'superieur.cpd@…',       nom: 'Responsable CpD'       },
    SUP_DIGITALE:  { email: 'superieur.digitale@…',  nom: 'Responsable Digitale'  },
...
| Champ | Description |
|-------|-------------|
| `sup` | **Clé** dans `PERSONNEL.superieurs` (ex. `'SUP_CPD'`), ou `null` |
...
Le thème (couleurs, police des emails) est résolu automatiquement par
`getThemeEmail(nomOrg, emailSup)` dans `Notifications.gs`.

Priorité de résolution :
1. `PRESIDENCE_MAP[emailSup]` — surcharge par supérieur (si présent)
2. Entrée de `PRESIDENCE_MAP` dont le `nomOrg` correspond
3. Défauts codés en dur (palette bleu-vert `#016579`)

Fond d'entête noir, texte blanc. Pour ajouter une organisation, ajouter une entrée
`PRESIDENCE_MAP` keyée par le `nomOrg`.
...
| S | ID Demande | Script | `MSK-2026-0001` |
...
|----------|---------|---------------|
| `SUP_PRES` | Supérieur → Présidence | Départements avec supérieur (CpD, Digitale, Technique) |
| `PRES` | Présidence directement | Département sans supérieur (SAF) |

Un **seul président par périmètre** (`PRES_GENERAL` ou `PRES_SAF`) : il valide ou
...
- **Président qui demande une absence** : la résolution de la présidence
  ...
  `PERSONNEL.presidentCroise` (ex. `PRES_GENERAL ↔ PRES_SAF`). Ce basculement
  s'applique de façon cohérente à **l'envoi du mail** et au **contrôle d'identité**.
...
1. Trouver la ligne (colonne S = référence `MSK-AAAA-XXXX`)
...
`name: nomOrg` affiche « Massaka SAS » comme nom d'expéditeur.
...
1. Vérifier que les emails de `PERSONNEL` sont les **vraies** adresses (pas les exemples `@massaka.com`)
2. Vérifier `SERVICE_SUP_MAP[dépt].presidence` et `PERSONNEL.presidentCroise`
...
Usage interne — Massaka SAS. Tous droits réservés.
